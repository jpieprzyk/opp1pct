...
dat$id_dat <- factor(dat$id_date)

require(ggplot2)
Sys.setlocale("LC_MESSAGES", 'pl_PL.UTF-8')

ggplot(dat, aes(x = id_date, y = opp_pct_pit)) + geom_bar(stat = "identity", fill = "#4682b4") +
  scale_y_continuous("procent podtak\uf3w PIT\n", limits = c(0, 0.0105), labels = scales::percent) +
